-- Drop and recreate functions, then add RLS policies
-- This fixes any parameter name conflicts
...
DROP FUNCTION IF EXISTS public.can_project_read(uuid, uuid);
DROP FUNCTION IF EXISTS public.can_project_write(uuid, uuid);
DROP FUNCTION IF EXISTS public.can_project_admin(uuid, uuid);
...
CREATE FUNCTION public.is_org_admin(uid uuid, org uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
...
CREATE FUNCTION public.can_project_read(uid uuid, pid uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
...
CREATE FUNCTION public.can_project_write(uid uuid, pid uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
...
CREATE FUNCTION public.can_project_admin(uid uuid, pid uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
